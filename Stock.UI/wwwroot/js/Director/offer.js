function GetOffer() {
    Get("Offer/ListOffersWithRequestStatus11", (data) => {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            html += `<td class="d-flex flex-row">
                           <button type="button" class="btn btn-danger btn-sm m-2" onclick='RetRequestStatus(${arr[i].requestId},13)'>Ret</button>
                        <button type="button" class="btn btn-success btn-sm m-2"  onclick='OnayRequestStatus(${arr[i].requestId},12)'>Onay</button>
                      </td>`;
            html += `</tr>`;
        }
        html += `</table>`;
        $("#divOfferDirector").html(html);
    });
}
